...
新增或修改跨进程能力时，先更新现有 role contract 与共享 payload/interface，再实现 main-owned behavior，并通过 executable loopback 和 production preload VM probe 验证参数、结果、错误、事件顺序、精确 unsubscribe 与角色不可见能力。Source-level Vitest 负责 contract/implementation 和 Playback lifecycle 行为；Build Verifier 只保留构建后的三角色执行、最小权限与 Reader Window Settings/route tracer，不镜像逐文件 source metadata 或重复业务场景。
...
Historical scope note: ADR-0036 supersedes only the typed `beforeInvoke` context-hook decision above. No accepted caller now needs sender identity, so production transport passes declared arguments directly; this decision's role declarations、fixed entrypoints、allow-list、channel direction and executable verification remain accepted.
