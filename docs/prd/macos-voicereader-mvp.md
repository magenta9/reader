...
VoiceReader 是一个 macOS menu bar 文字转语音工具。用户希望在任意应用中选中文本或复制文本后，通过全局快捷键或菜单栏入口快速朗读当前 Reading Target，并能在本机按时间回看曾经朗读过的全文历史。

Chrome 扩展不再是后续主线。VoiceReader 的 MVP 聚焦 macOS Selected Text / Clipboard Text reading、全局快捷键、系统级播放浮层、本地历史和本地设置。
...
用户界面使用中文，产品名显示为 `VoiceReader`。主窗口参考 Typeless 的结构和操作方式，但不复制品牌元素：左侧导航、内容面板、历史记录分栏和简洁设置页。界面跟随 macOS 系统外观。
...
主窗口默认打开 `主页`；从菜单栏的 `历史记录` 或 `设置` 入口打开时直接跳到对应页。默认窗口尺寸为 `1100x760`，最小尺寸为 `900x620`。
...
主页不显示当前 Selected Text 或 Clipboard Text 全文，不显示当前播放状态，不显示历史列表。当前 Reading Target 播放的唯一视觉反馈是系统级 Playback Overlay。

`播放` 始终可见。配置未完成时按钮 disabled，并在配置状态区域说明原因。快捷键和菜单栏 Play 在配置未完成时静默跳过。
...
当前 Reading Target 播放时，Play 找到有效 Selected Text 或 Clipboard Text 后立即显示系统级 Playback Overlay。准备阶段显示较弱波形，开始播放后显示 active 波形。播放完成后快速淡出。失败时直接消失。

Playback Overlay 是单独的 Electron BrowserWindow：

- 固定在鼠标当前所在显示器底部中央，使浮层跟随用户当前工作页面
- 不抢焦点
- 不显示任何文本
- 只显示真实振幅驱动的波形和一直可见的 `×`
...
VoiceReader 使用 SQLite 存储本地 Reading History、设置元数据、Error Log 和 MiniMax API key，数据库位于 Electron app data 目录。SQLite 不加密。MiniMax API key 直接保存到 SQLite，不使用 Electron `safeStorage` 或 macOS Keychain，避免钥匙串访问提示。
