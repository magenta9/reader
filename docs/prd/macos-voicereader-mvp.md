...
VoiceReader 是一个 macOS menu bar 文字转语音工具。用户希望在任意应用中复制文本后，通过全局快捷键或菜单栏入口快速朗读当前剪切板文本，并能在本机按时间回看曾经朗读过的全文历史。

Chrome 扩展不再是后续主线。VoiceReader 的 MVP 聚焦 macOS clipboard reading、全局快捷键、系统级播放浮层、本地历史和本地设置。
...
用户界面使用中文，产品名显示为 `VoiceReader`。主窗口参考 Typeless 的结构和操作方式，但不复制品牌元素：左侧灰底导航、内容白色面板、底部齿轮入口、历史记录分栏和简洁设置页。界面跟随 macOS 系统外观。

## Core Concepts

- **Clipboard Text**: 用户显式触发 Play 时，当前剪切板里的非空纯文本。
- **Reading Target**: 本次要朗读的文本；macOS MVP 中来源只来自 Clipboard Text。
...
- **Playback Overlay**: 当前 Clipboard Text 播放时出现的系统级浮层。History Replay 不使用系统级浮层。
- **History Replay**: 从历史详情发起的重播，播放整条历史全文，不新增历史记录。

## User Stories

1. 作为 macOS 用户，我想复制文本后按全局快捷键播放，这样不用打开浏览器扩展或手动粘贴到别的工具。
2. 作为 macOS 用户，我想通过菜单栏菜单点击播放当前剪切板文本，这样忘记快捷键时也能使用。
...
8. 作为 macOS 用户，我想使用自己的 MiniMax API key，这样 VoiceReader 使用我自己的订阅。
9. 作为 macOS 用户，我想选择 Voice、Speech Rate 和 Model，这样可以调整朗读体验。
10. 作为 macOS 用户，我想 VoiceReader 登录后自动启动可选，这样常用时可以随时按快捷键使用。
...
底部齿轮入口打开并聚焦主窗口的 `设置` 页，不创建单独设置窗口。主窗口默认打开 `主页`；从菜单栏的 `历史记录` 或 `设置` 入口打开时直接跳到对应页。默认窗口尺寸为 `1100x760`，最小尺寸为 `900x620`。
...
主页不显示当前 Clipboard Text 全文，不显示当前播放状态，不显示历史列表。当前 Clipboard Text 播放的唯一视觉反馈是系统级 Playback Overlay。

`Play Clipboard` 始终可见。配置未完成时按钮 disabled，并在配置状态区域说明原因。快捷键和菜单栏 Play 在配置未完成时静默跳过。
...
默认 Activation Shortcut 为 `Cmd+Shift+R`，设置页允许用户修改。Electron 不区分左/右 Shift，因此不支持 `Cmd+右Shift+R` 作为精确默认快捷键。

Stop Shortcut 固定为 Esc，不提供配置。Esc 只在 Playback Session 活跃时全局生效。

播放中触发新的 Play 或 Activation Shortcut 时，立即停止当前 Playback Session，并用最新 Clipboard Text 开始新的 Playback Session。

播放中只允许取消，不支持暂停或继续。取消后若要再播放，需要重新从头开始。Esc 和 Overlay 的 `×` 都是用户主动停止，不写 Error Log，也不改变 Reading History Record。

取消或新播放替换时必须尽快 abort 当前 MiniMax streaming，并清空待播放音频。

## Playback Overlay

当前 Clipboard Text 播放时，Play 找到有效 Clipboard Text 后立即显示系统级 Playback Overlay。准备阶段显示较弱波形，开始播放后显示 active 波形。播放完成后快速淡出。失败时直接消失。
...
- 不显示任何文本
- 只显示真实振幅驱动的波形和一直可见的 `×`
- hover 时显示胶囊式估算进度条，左侧深色填充表示大概比例
- 不显示播放进度文字
...
历史记录不可编辑，不支持选中部分播放。删除单条需要轻量二次确认；删除后选中下一条最近记录，没有记录则显示空状态。
...
History Record 在 Play 找到有效 Clipboard Text 后、调用 MiniMax 前创建。即使后续 MiniMax 失败，该记录仍保留。MiniMax 成功/失败状态不写入 History Record。

重复播放同一 Clipboard Text 时，若 5 分钟内文本完全相同，则复用最近记录，不新增重复记录。超过 5 分钟后再次从 Clipboard Text 播放同一文本，可以创建新记录。
...
设置页提供 `Clear Reading History`，使用页内确认，不用系统 modal。MVP 不做导出、备份、收藏或单条永久保留。
...
VoiceReader 使用 SQLite 存储本地 Reading History、设置元数据和 Error Log，数据库位于 Electron app data 目录。SQLite 不加密。MiniMax API key 使用 Electron `safeStorage` 加密后保存到本地配置，不明文放入 SQLite。
...
- 主进程负责 clipboard、globalShortcut、menu bar、窗口、overlay、SQLite、safeStorage、MiniMax streaming 和 IPC 协调。
...
- Integration test overlay lifecycle：有效文本后显示、完成淡出、失败消失、Esc/`×` 停止。
- Integration test settings persistence：API key safeStorage、shortcut、Speech Rate、Model、retention、Launch at Login。
- UI verification should include Home, History list/detail, Settings, Playback Overlay, and dark/light appearances.
