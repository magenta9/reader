...
- Production Runtime Role Binding：一个固定三角色 owner 独占 Reader Window、Playback Renderer、Playback Overlay 的 preload source/artifact、document artifact、`voiceReader` exposure 与 Electron web-preference policy；build、preload 和 main-owned window adapters 都消费该 owner，不通过 URL 或 pathname 推断权限。
- Preload bridge：三个 runtime 各自使用 Production Runtime Role Binding 指定的最小 preload artifact；role-scoped executable contract 同时生成 renderer bridge 与 main wiring，renderer 不直接使用 Node API。
...
- Record Workspace：Reader Window 每次进入 History 或 Favorites 都创建一个 renderer-owned workspace。它通过两个窄 adapter 复用同一个访问 owner，独占列表读取、倒序分组、默认/相邻选择、详情 Replay、复制、添加收藏、删除、反馈、single-flight 与访问代际；React view 只订阅不可变 snapshot、发送语义 intent，并适配焦点。App 只保留 10 秒跨路由 undo host，不接管 Record workflow。
...
- `src/renderer/App.tsx`：可注入 fake bridge 的 Reader Window React UI；Home、Settings 与 Record surfaces 只作为对应 workspace 的渲染、语义 intent、导航与 DOM/focus adapter；App 额外承载跨路由 undo host。
...
- `src/renderer/record-workspace.ts`：History/Favorites 共用的路由访问 workflow owner，以 kind adapter 隔离差异，提供不可变分组/选择 snapshot、Record intents、undo callback 与跨访问迟到响应隔离。
...
- `src/shared/production-runtime-role-bindings.json`、`production-runtime-role-bindings.ts`：Production Runtime Role Binding 的规范三角色源码/构建身份；独占 preload source/artifact、document、exposure 与 web-preference policy。
- `src/shared/production-runtime-role-identity.ts`：无 source metadata 与顶层副作用的运行时 identity parser/resolver；packaged main 只用它验证生成的 runtime adapter，不会把源码矩阵嵌入 main bundle。
...
- `dist/`：唯一可发布 Build Product，只含 main、三个 preload、三个 renderer 的运行 bundle/HTML/CSS/source maps、规范 `runtime-role-bindings.json`、main 实际加载的 `runtime/production-runtime-role-bindings.cjs`、图标资源与 macOS native addon；不含 TypeScript 内部模块树。
