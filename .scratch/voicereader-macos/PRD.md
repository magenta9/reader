...
- Store Reading History, settings metadata, Error Log, and the MiniMax API key in local SQLite; do not encrypt SQLite.
...
- Integration test Play/Stop/replacement IPC, MiniMax abort behavior, overlay lifecycle, settings persistence, SQLite key storage, and History Replay behavior.
