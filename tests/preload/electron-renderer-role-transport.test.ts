...
describe("Electron renderer role transport", () => {
  it("preserves invoke arguments and removes the exact navigate listener", async () => {
    ...
    const invoke = vi.fn(async () => ({ route: "favorites" as const, revision: 3 }));
    ...
    await expect(bridge.setRoute("favorites")).resolves.toEqual({
      route: "favorites",
      revision: 3
    });
    const unsubscribe = bridge.onNavigate(navigate);
    listeners
      .get(APP_SHELL_CHANNELS.navigate)
      ?.({} as never, { route: "settings", revision: 4 });
    unsubscribe();

    expect(invoke).toHaveBeenCalledWith(APP_SHELL_CHANNELS.setRoute, "favorites");
    expect(navigate).toHaveBeenCalledWith({ route: "settings", revision: 4 });
