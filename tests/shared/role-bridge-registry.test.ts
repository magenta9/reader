...
describe("role bridge registry", () => {
  ...
  it("keeps transport invocation details outside role implementations", async () => {
    let addHandler: ((args: readonly unknown[]) => unknown) | undefined;
    registerRoleHandlers(
      fixtureContract,
      {
        add: (left, right) => left + right,
